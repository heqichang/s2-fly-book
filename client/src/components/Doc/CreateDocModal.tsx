...
function CreateDocModal({ visible, onClose, onCreated, teamId, folderId }: CreateDocModalProps) {
  ...
  const validateForm = (): boolean => {
    ...
    if (title.trim() && title.length > 100) {
      newErrors.title = '文档标题不能超过100个字符'
    }
    setErrors(newErrors)
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={handleClose}>
        <div className="bg-white rounded-2xl shadow-2xl w-[440px] p-6" onClick={(e) => e.stopPropagation()}>
          <h3 className="text-lg font-semibold text-gray-900 mb-6">创建文档</h3>
          <form onSubmit={handleSubmit}>
            <div className="mb-6">
              ...
              <input
                ...
                className={`w-full px-4 py-2.5 bg-gray-50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#3370FF]/20 focus:bg-white transition-all ${errors.title ? 'ring-2 ring-red-500/30' : ''}`}
                maxLength={100}
                autoFocus
              />
              {errors.title && (
